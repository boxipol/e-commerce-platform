...
public class ProductMigration {
	...
	public static void main(String[] args) {
		try (CqlSession session = CqlSession.builder()
			...
			// Source query
			String selectQuery = "SELECT category, created_at, product_id, name, brand, price, stock " + "FROM products_by_id";
			ResultSet rs = session.execute(selectQuery);

			// Prepared insert (important for performance)
			PreparedStatement insertStmt = session.prepare("INSERT INTO products_by_category " + "(category, created_at, product_id, name, brand, price, stock) " + "VALUES (?, ?, ?, ?, ?, ?, ?)");
			BatchStatementBuilder batch = BatchStatement.builder(DefaultBatchType.UNLOGGED);
			int count = 0;

			for (Row row : rs) {
				batch.addStatement(insertStmt.bind(row.getString("category"), row.getInstant("created_at"), row.getUuid("product_id"), row.getString("name"), row.getString("brand"), row.getBigDecimal("price"), row.getInt("stock")));
				count++;
